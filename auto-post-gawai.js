 ...
 * auto-post-sains.js
 * ---------------------------------------------------------------
 * Auto-posting KONTEN GAWAI ke WordPress (JNews) — gaya
...
const KATEGORI_GAWAI_ID = parseInt(process.env.KATEGORI_GAWAI_ID || '1', 10); // ID kategori "Gawai" di WordPress
const POST_STATUS_GAWAI = process.env.POST_STATUS_GAWAI || 'draft';
const MAKS_GAWAI_PER_PROSES = parseInt(process.env.MAKS_GAWAI_PER_PROSES || '2', 10);
...
const LOG_FILE = './posted-gawai-log.json'; // dedup terpisah dari modul lain
// -----------------------------------
...
async function ambilOgImage(url) {
  try {
    const res = await axios.get(url, {
      timeout: 15000,
      headers: { 'User-Agent': 'Mozilla/5.0 (auto-post-gawai bot)' },
    });
...
async function unggahFotoDenganKredit(urlGambar, sourceName) {
  ...
  try {
    ...
    const namaFile = `gawai-${Date.now()}.${ekstensi}`;
    const auth = Buffer.from(`${WP_USER}:${WP_APP_PASSWORD}`).toString('base64');
 ...
 */
async function tulisArtikelGawaiDenganLuna(item) {
  const systemPrompt = `Anda adalah penulis gawai dan teknologi untuk portal Probaca.com. Gaya tulisan Anda seperti media teknologi yang bikin pembaca tertarik.
Tugas Anda: mengubah materi sumber menjadi artikel teknologi berbahasa Indonesia yang seru dibaca orang awam,
...
TAG: <PERSIS 4 kata kunci/frasa pendek dipisah koma, mewakili topik utama - — mis. isu, "gawai", "gadget", topik>
...
async function postingKeWordPress({ judul, isi, sourceLink, sourceName, foto, ringkasan, tagIds }) {
  ...
  const payload = {
    ...
    status: POST_STATUS_GAWAI,
    categories: [KATEGORI_GAWAI_ID],
  };
...
async function main() {
  console.log('Mengecek konten gawai baru...');
  const semuaItemBaru = await fetchNewItems();
  console.log(`Ditemukan ${semuaItemBaru.length} item baru total.`);

  const items = semuaItemBaru.slice(0, MAKS_GAWAI_PER_PROSES);
  if (semuaItemBaru.length > items.length) {
    console.log(`Memproses ${items.length} dulu (batas MAKS_GAWAI_PER_PROSES=${MAKS_GAWAI_PER_PROSES}); sisanya menyusul di jadwal berikutnya.`);
  }

  for (const item of items) {
    try {
      console.log(`Memproses: ${item.title}`);
      const artikel = await tulisArtikelGawaiDenganLuna(item);
      ...
      console.log(`Berhasil dibuat sebagai "${POST_STATUS_GAWAI}" -> ID: ${hasil.id}${foto ? ' (dengan foto)' : ''}${tagIds.length ? ` (${tagIds.length} tag)` : ''}`);
